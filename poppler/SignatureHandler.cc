...
{
  fprintf(stderr,"getSignerName begin()\n");
  if (!CMSSignerInfo)
      return NULL;

  fprintf(stderr,"getSignerName end()\n");
...
{
  if (cms_item->data){
    fprintf(stderr,"CMSMessage is not null %d and tyoe:%d\n",cms_item->len,cms_item->type);
    return NSS_CMSMessage_CreateFromDER(cms_item, NULL, NULL /* Content callback */
                        , NULL, NULL /*Password callback*/
                        , NULL, NULL /*Decrypt callback*/);
  } else {
    fprintf(stderr,"CMSMessage is NULL\n");
    return NULL;
  }
}
...
NSSCMSSignedData *SignatureHandler::CMS_SignedDataCreate(NSSCMSMessage * cms_msg)
{
	int nlevels, i;
	if(cms_msg==NULL) {
		fprintf(stderr,"cms msg is NULL\n");
	}
    nlevels = NSS_CMSMessage_ContentLevelCount(cms_msg);
    fprintf(stderr,"nlevels: %d\n",nlevels);
    for (i = 0; i < nlevels; i++) {
        NSSCMSContentInfo *cinfo;
        SECOidTag typetag;

        cinfo = NSS_CMSMessage_ContentLevel(cms_msg, i);
        typetag = NSS_CMSContentInfo_GetContentTypeTag(cinfo);
        fprintf(stderr,"Typetag: %d\n", typetag);
    }  
